...
func (api *GoMdAPI) RegisterSpi(spi GoMdSPI) {
	//C.SetRegisterMdSpiUserID(spi.MdSPI, C.CString(api.UserID))
	//spi.SetGoMdApi(api)
	log.Debug("%+v", *api)
	//log.Debug("%+v", unsafe.Pointer(api))
	//api.Spi = uintptr(unsafe.Pointer(&spi))
	//pSpi := &(uintptr(unsafe.Pointer(&spi)))
	C.RegisterMdSpi(api.MdAPI, spi.CMdSPI())
	mdUserSpi = spi
	//C.RegisterMdSpi(uint(uintptr(unsafe.Pointer(api))), api.MdApi, spi.CMdSpi())
	//initUserApiInfo
}
...
func (api *GoMdAPI) RegisterFront() {
	log.Debug("连接前置服务器：", api.FrontAddr)
...
//func createFlowPath(userId string) string {
//	flowPath := filepath.Join("/data/md/flowpath", userId)
//	if _, err := os.Stat(flowPath); os.IsNotExist(err) {
//		log.Info("create flowpath: ", flowPath)
